...
async def test_translate_video_ya() -> None:
    client = VOTClient()
    video_data = VideoData(url="https://youtu.be/dQw4w9WgXcQ", videoId="dQw4w9WgXcQ", host="youtube")

    # Mocks for create_session (already tested above)
...
def test_sync_client_wrapper() -> None:
    with VOTClientSync() as sync_client:
        ...
        trans_res = MagicMock()
        sync_client.client.translate_video = AsyncMock(return_value=trans_res)

        res = sync_client.translate_video(video_data)
        assert res == trans_res
        sync_client.client.translate_video.assert_called_once_with(
            video_data,
            request_lang=None,
            response_lang=None,
