...
`yavot-py` — это современная, быстрая и полностью типизированная библиотека на Python для взаимодействия с **Yandex Video Translation API** (Яндекс Закадровый Перевод Видео). Данная библиотека является портом популярной TypeScript-библиотеки `vot.js`.
...
Пакет поставляется со встроенной консольной утилитой, которая опрашивает API в реальном времени, скачивает аудиофайл перевода, а также может сохранять и конвертировать субтитры.

### Запуск CLI
В зависимости от вашего окружения и способа установки, утилиту можно запустить следующими способами:
- **Если пакет установлен в системе/окружении:**
  ```bash
  vot <аргументы>
  ```
- **Локально (для разработки, с помощью вспомогательного скрипта):**
  ```bash
  ./vot-cli <аргументы>
  ```
- **Напрямую через Python:**
  ```bash
  python -m vot.cli <аргументы>
  # Или через uv:
  uv run python -m vot.cli <аргументы>
  ```

### Аргументы командной строки

| Опция | Короткий флаг | Описание | По умолчанию |
|-------|---------------|----------|--------------|
| `url` | *Позиционный* | URL видео для перевода (поддерживаются YouTube, Vimeo, Twitch, VK, TikTok и др.) | *Обязательный* |
| `--lang-from` | `-f` | Исходный язык видео (например, `en`, `de`, `zh` или `auto`) | `en` |
| `--lang-to` | `-t` | Язык перевода (например, `ru`, `en`, `kk`) | `ru` |
| `--output` | `-o` | Локальный путь для сохранения скачанного аудиофайла перевода | Нет |
| `--subtitles` | `-s` | Запросить и вывести в консоль ссылки на доступные субтитры | `False` |
| `--output-subs` | | Локальный путь для сохранения переведенных субтитров (поддерживаются форматы `.srt`, `.vtt`, `.json`) | Нет |
...
2. **Перевести видео с немецкого на русский и скачать аудиофайл перевода локально:**
   ```bash
   vot https://www.youtube.com/watch?v=dQw4w9WgXcQ -f de -t ru -o my_translation.mp3
   ```

3. **Запросить перевод и вывести ссылки на субтитры:**
   ```bash
   vot https://www.youtube.com/watch?v=dQw4w9WgXcQ -s
   ```

4. **Скачать субтитры с автоматической конвертацией в SRT:**
   ```bash
